...
class Curve:
  ...
  @property
  def lines(self) -> list[WindowLineObject]:
    return []
...
class Wireframe:
  ...
  def window_objects(self) -> list[WindowObject]:
    ...
    for curve in self.curves: objects.extend(curve.lines)
    for surface in self.surfaces: objects.extend(surface.window_objects)  # type: ignore
...
# class PointObject(PastWireframe):
...
# class LineObject(PastWireframe):
...
# class PolygonObject(PastWireframe):
...
# class CurveObject_2D(PastWireframe):
...
# class PolyhedronObject(PastWireframe):
